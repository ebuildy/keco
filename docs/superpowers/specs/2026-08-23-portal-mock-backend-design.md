...
### Layer 5 — the service worker script never enters the tree

MSW needs its worker script served from the web root, and `vite build` copies `public/`
verbatim into `dist/`. So `apps/web/public/mockServiceWorker.js` is **generated by `msw init`
as a step of the `web:mock` task and gitignored**, never committed. It does not exist in a
clean checkout, a CI checkout, or a deployment build, so there is nothing for `vite build` to
copy. Layer 4's scan catches it if it ever reappears.
