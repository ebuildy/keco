...
deployment build. Gitignoring it does exactly that — it does not remove a copy already
generated on a developer's machine by an earlier `mise run web:mock`; `mise run build` handles
that case — `mise run build` deletes it before `vite build` runs, as hygiene — and Layer 4's
scan is the actual backstop either way.
Append to `.gitignore`, after the `.data/` line:

```gitignore
# MSW worker script — generated by `mise run web:mock`, dev-only. Kept out of a clean checkout,
# CI and deployments; `mise run build` also drops any locally generated copy before building.
