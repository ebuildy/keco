...
> bodies below.
>
> **Amendment, 2026-09-08 — `seeds/` renamed to `sources/`.** Task 5 (and, before the amendment
> above, Tasks 1–4, 8–14) built everything under `crawler/seeds/`. With every ref-yielding
> adapter gone, `crawler/seeds/github/` — Task 5's move target, Tasks 2–4 and 8's per-repo
> pipeline — was the only thing left under `seeds/`, so it was renamed to `crawler/sources/
> github/`. Every `seeds/github/…` path in the task bodies below (and every deleted
> `crawler/seeds/{cncf,krew,artifacthub,awesome,index,repo-url}.ts` from the 2026-09-07 amendment,
> which never existed under `sources/` at all) is left as originally written.
