...
/**
 * Chatbot (v2, §11) — strictly retrieval-grounded, and the rules are not negotiable:
 *   1. retrieve first (Meilisearch hybrid), answer only from what came back
 *   2. never name a tool outside the retrieved set; never emit an install command that is
 *      not in that tool's verified install_methods — the model does not write shell
